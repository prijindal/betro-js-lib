...
const generateExchangePair = async () => {
  ...
  const publicKey = await crypto_1.default.subtle.exportKey(
    "spki",
    keys.publicKey
  );
  const privateKey = await crypto_1.default.subtle.exportKey(
    "pkcs8",
    keys.privateKey
  );
  return {
...
const importEcdhKey = (format, key) =>
  crypto_1.default.subtle.importKey(
    format,
